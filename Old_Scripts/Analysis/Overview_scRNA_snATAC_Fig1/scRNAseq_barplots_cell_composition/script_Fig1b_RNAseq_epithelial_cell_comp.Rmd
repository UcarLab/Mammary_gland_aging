---
title: "Cell composition plot for epithelial cells"
...
opts_knit$set(root.dir = "/Users/katiyn/Dropbox (JAX)/Mammary_aging_clean/code/") #set root dir!
```
...
```{r input_object, include=TRUE, echo=TRUE, eval=FALSE}
#Call function
...
```{r table_rep_cluster, include=TRUE, echo=TRUE, eval=FALSE}
cells_per_ident_per_cluster_reps =table(Idents(scRNA_seq_expt), scRNA_seq_expt@meta.data$replicate)
write.table(cells_per_ident_per_cluster_reps, "./results/Barplots_cell_composition/Table_cells_per_replicate_per_cluster.txt", sep="\t", quote=FALSE)
```

```{r cell_comp_epithelial, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r convert_format_calc_percent, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r calc_mean_groups, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r calc_SEM_display_plot, include=TRUE, echo=TRUE, eval=FALSE}
# Calc SEM  
...
plotSEM <- plot + geom_errorbar(data=means.sem, aes(ymax=upper,  ymin=lower), 
                                width=0.15)
png("./results/Barplots_cell_composition/Plot_epithelial_SEM.png")
plotSEM
dev.off()
```

```{r qc_plots, eval=TRUE, warning= FALSE, echo=FALSE, out.width="60%", fig.align = "center", fig.cap= "QC plots", warning=FALSE}
knitr::include_graphics("./results/Barplots_cell_composition/Plot_epithelial_SEM.png")
```
