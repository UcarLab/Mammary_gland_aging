...
def Proportion(sid, Figname):
    adata = sc.read_h5ad(DIR+"{}".format(sid))    # Marked ST data From 01_Main_indi_.py
    if sid.startswith("Y"):
        non_imm = adata[adata.obs["annotation"]!="Immune",:].copy()
        max_value = non_imm.obs["Cd3/8_Pdcd1"].sum() / non_imm[non_imm.obs["annotation"]=="Epithelial"].obs.shape[0]
        dp = sc.pl.dotplot(non_imm,
                           var_names=["Cd3/8_Gzmk","Cd3/8_Pdcd1","Gamma-deltaT"],
                           groupby=['annotation'],
                           save=+f"Supp_Fig9E_{sid}",
                           dot_max=max_value)  # Adjust dot_max 
    ...
    def Cal_(df, cell):
        ...
        def Plot(input,sam_id):
            ...
            plt.xticks([])
            plt.xlabel(cell)
            plt.savefig(FIG_OUT+"{}_{}_{}_{}.pdf".format(Figname, sam_id[0], sam_id, cell.replace("/","_")),
                        bbox_inches="tight")
...
o1, o1p, o1z, o1t = Proportion("O1.h5ad_test_new", "Supp_Fig9F")
o2, o2p, o2z, o2t = Proportion("O2.h5ad_test_new", "Supp_Fig9F")
y1, y1p, y1z, y1t = Proportion("Y1.h5ad_test_new", "Supp_Fig9F")
y2, y2p, y2z, y2t = Proportion("Y2.h5ad_test_new", "Supp_Fig9F")
# %%
merged = np.vstack([o1,o2,y1,y2])
merged_p = np.vstack([o1p,o2p,y1p,y2p])
merged_z = np.vstack([o1z,o2z,y1z,y2z])

def Draw_merged_bar(merged, Figname, *ext):
    ...
    sns.despine()
    plt.ylim(bottom=0)
    plt.savefig(FIG_OUT+"{}_{}.pdf".format(Figname, ext[1]),
                bbox_inches="tight")

# Draw_merged_bar(merged_p, "p")
Draw_merged_bar(merged_z, "Supp_Fig9G", "z", "Z-score")
Draw_merged_bar(merged, "Supp_Fig9D", "np", "Proportion of Epithelial \n (Of whole spots)")
# %%
