...
Celltypes = c('Naive_CD4-1','Naive_CD4-2','Naive_CD8-1','Naive_CD8-2','CD8_GZMK','CD8_GZMM','MemCD4',"GD_MAIT","CD8_ISG","NK")
df <- chromVAR.signature.motifs %>%
  mutate(CellType = factor(CellType, levels = Celltypes)) %>%
  mutate(Log_Adj_P = -log10(adj_p_val)) %>%
  mutate(Log_Adj_P = ifelse(Log_Adj_P > 250, 250, Log_Adj_P)) %>%
...
pdf("~/path/to/pdf", height = 5, width = 15)
pheatmap::pheatmap(c[,dhc$labels], 
                   cluster_rows = F,
                   cluster_cols = F,
                   scale = "none", 
                   na_col = "grey",
