library(VennDiagram)
## Needs list of differential peaks
LumAV <- read.csv("/path/to/Luminal-AV diff peaks", sep = '\t')
LumHS <- read.csv("/path/to/Luminal-HS diff peaks", sep = '\t')
Myo <- read.csv("/path/to/Myoepithelial diff peaks", sep = '\t')
...
p1 <- venn.diagram(
  x = list(LumAV$peak, LumHS$peak, Myo$peak),
